...
\newcommand{\codeblockwrapper}[4]{%
  \begin{tcolorbox}[
      ...
      overlay={%
        \ifstrequal{#4}{true}{%
          \node[anchor=north east,font=\tiny\ttfamily,text=gray] at ([xshift=-5pt,yshift=-3pt]frame.north east) {#2};%
        }{}%
      }
    ]%
    \inputminted[fontsize=\footnotesize,breaklines=true,linenos=false]{#2}{#1}%
  \end{tcolorbox}%
}

\newcommand{\codeblockplain}[1]{%
  \begin{tcolorbox}[
      colback=codeBlockBg,
      colframe=codeBlockBorder,
      boxrule=0pt,
  ...
  rendererPrototypes={
    ...
    inputFencedCode = {%
      ...
      {%
        ...
        \codeblockwrapper{#1}{text}{20pt}{true}%
      }%
      \else
      \codeblockwrapper{#1}{#3}{20pt}{true}%
      \fi
      \fi
    },
    inputVerbatim = {
...
\begin{center}
  ...
  \begin{tcolorbox}[
    ...
    {\fontsize{36}{40}\selectfont\bfseries\color{titleMainText}\assignmenttitle\par}
    \vspace{0.4cm}
    {\fontsize{16}{20}\selectfont\color{titleSubText}\coursename\par}  \end{tcolorbox}
\end{center}
\vspace*{\fill}
\ifenablepagecredits
\vfill
